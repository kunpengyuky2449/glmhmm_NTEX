...
def plot_glmhmm_weights(weights, ax, labels, title):
    M_temp = weights.shape[2]
    k_temp = weights.shape[0]
    cols = [
        '#ff7f00', '#4daf4a', '#377eb8', '#f781bf', '#a65628', '#984ea3',
        '#999999', '#e41a1c', '#dede00'
    ]
    for kk in range(k_temp):
        ax.plot(
            range(M_temp),
            weights[kk][0][:],
            marker='o',
            label="state " + str(kk + 1),
            color=cols[kk],
            lw=1,
            alpha=0.7)
    ax.set_xticks(range(M_temp))
    ax.set_xticklabels(
        labels,
        fontsize=10)
